...
int findMinInRangeBruteForce(vector<int>& arr, int i, int j) {
    int idx = i;
    int max_value = arr[idx];
    while (idx <= j) {
        max_value = min(arr[idx], max_value);
        idx++;
    }
    return max_value;
}
...
int main() {
    ...
    // Example query
    int result = segmentTree.findMinInRange(3, 8, n);
    cout << result << endl;
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            ...
            if (segmentTree.findMinInRange(i, j, n) != findMinInRangeBruteForce(arr, i, j)) {
                cout << "MISMATCH" << endl;
            }
        }
    }
